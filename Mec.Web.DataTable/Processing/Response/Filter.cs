...
namespace Mec.Web.DataTable.Processing.Response
{
    internal static class Filter
    {
        ...
        internal static DateTimeOffset ToDateTime(string value)
        {
            ...
            if (MecDataTableOptions.Instance.RequestDateTimeFormatType == DateTimeFormatType.Auto && DateTimeOffset.TryParse(value, out var result))
            {
                result = result.DateTime.WithTimeZone(MecDataTableOptions.Instance.DateTimeTimeZone);

                return result;
            }

            if (DateTimeOffset.TryParseExact(value, MecDataTableOptions.Instance.DateTimeFormat, CultureInfo.InvariantCulture, DateTimeStyles.AssumeUniversal, out var dateTime))
            {
                result = dateTime;
            }
            else if (DateTimeOffset.TryParseExact(value, MecDataTableOptions.Instance.DateFormat, CultureInfo.InvariantCulture, DateTimeStyles.AssumeUniversal, out var date))
            {
                result = date;
            }
            else
