...
(function ($) {
    ...
    $.fn.columnFilter = function (options) {
        ...
        function fnCreateRangeInput(oTable) {
            ...
            var from = $('<input type="text" class="number_range_filter form-control" id="' +
                sFromId +
                '"  value = "' +
                fromDatePre +
                '" rel="' +
            ...
            var to = $('<input type="text" class="number_range_filter form-control" id="' +
                sToId +
                '" value="' +
                toDatePre +
                '" rel="' +
        ...
        function fnCreateDateRangeInput(oTable, pickerFunctionName) {
            ...
            //th.html(_fnRangeLabelPart(0));
            var sFromId = oTable.attr("id") + "_range_from_" + i;
            var from = $('<input type="text" class="date_range_filter form-control" id="' +
                sFromId +
                '" value="' +
                fromDatePre +
                '" rel="' +
            ...
            //th.append(_fnRangeLabelPart(1));
            var sToId = oTable.attr("id") + "_range_to_" + i;
            var to = $('<input type="text" class="date_range_filter form-control" id="' +
                sToId +
                '" value="' +
                toDatePre +
                '" rel="' +
        ...
        function fnCreateColumnSMec(oTable, aData, iColumn, nTh, sLabel, bRegex, oSMeced, bMultisMec) {
            if (aData == null)
            ...
            for (j = 0; j < iLen; j++) {
                if (typeof (aData[j]) != "object") {
                    ...
                    if (escape(aData[j]) == currentFilter || escape(aData[j]) == escape(currentFilter))
                        sMeced = "sMeced ";
                    r += "<option " + sMeced + ' value="' + escape(aData[j]) + '">' + aData[j] + "</option>";
                } else {
                    var sMeced = "";
                    //HMif (("^" + escape(aData[j]) + "$") == currentFilter) sMeced = 'sMeced '
                    //HMr += '<option ' + sMeced + 'value="' + escape(aData[j].value + "$") + '">' + aData[j].label + '</option>';

                    if (bRegex) {
                        //Do not escape values if they are explicitely set to avoid escaping special characters in the regexp
                        if (aData[j].value == currentFilter) sMeced = "sMeced ";
                        r += "<option " + sMeced + 'value="' + aData[j].value + '">' + aData[j].label + "</option>";
                    } else {
                        if (escape(aData[j].value) == currentFilter) sMeced = "sMeced ";
                        r += "<option " +
                            sMeced +
                            'value="' +
            ...
            if (bMultiselect) {
                select.change(function () {
                    ...
                    var sMecedOptions = $(this).val();
                    var asEscapedFilters = [];
                    if (sMecedOptions == null || sMecedOptions == []) {
                        var re = "^(.*)$";
                    } else {
                        $.each(sMecedOptions,
                            function (i, sFilter) {
        ...
        function fnCreateSMec(oTable, aData, bRegex, oSMeced, bMultisMec) {
            var oSettings = oTable.fnSettings();
            if ((aData == null || typeof (aData) == "function") &&
                    ...
                    "fn": (function (iColumn, nTh, sLabel) {
                        return function (oSettings) {
                            ...
                            if (oSettings.iDraw == 2 &&
                                ...
                                oSettings.sAjaxSource != "" &&
                                !oSettings.oFeatures.bServerSide) {
                                return fnCreateColumnSMec(oTable,
        ...
        function fnCreateDropdown(aData) {
            var index = i;
            var r =
                '<div class="dropdown sMec_filter form-control"><a class="dropdown-toggle" data-toggle="dropdown" href="#">' +
                label +
        ...
        function fnCreateCheckbox(oTable, aData) {
            ...
            //clean the string
            var localLabel = label.replace("%", "Perc").replace("&", "AND").replace("$", "DOL").replace("£", "STERL")
                .replace("@", "AT").replace(/\s/g, "_");
            ...
            r += '<button id="' +
                buttonId +
                '" class="checkbox_filter btn btn-default" > ' +
                labelBtn +
                "</button>"; //filter button witch open dialog
            r += '<div id="' +
                checkToggleDiv +
                '" ' +
                'title="' +
            ...
            for (j = 0; j < iLen; j++) {
                ...
                checkbox.change(function () {
                    ...
                    $('input:checkbox[name="' + localLabel + '"]:checked').each(function (index) {
                        ...
                        //concatenation for sMeced checks in or
                ...
                buttons: [
                    {
                        text: "Reset",
                        click: function () {
                            //$('#'+buttonId).removeClass("filter_sMeced"); //LM remove border if filter sMeced
        ...
        return this.each(function () {
            ...
            for (j = 0; j < aiCustomSearch_Indexes.length; j++) {
                ...
                var fnSearch_ = function () {
                    ...
                    return $("#" + id + "_range_from_" + aiCustomSearch_Indexes[j]).val() +
