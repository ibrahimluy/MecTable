using Mec.Core.DictionaryUtils;
...
namespace Mec.Web.DataTable.Utils.DataTableModelUtils
{
    public static class DataTableModelExtensions
    {
        ...
        private static IEnumerable<JObject> ConvertColumnToTargetedProperty<TProperty>(string jsonPropertyName,
            Func<ColumnModel, TProperty> propertySMecor, Func<TProperty, bool> propertyPredicate,
            params ColumnModel[] columns)
        {
            return ConvertColumnToTargetedProperty(jsonPropertyName, propertySMecor, propertyPredicate, x => x,
                columns);
        }
            ...
            Func<TProperty, TResult> propertyConverter, params ColumnModel[] columns)
        {
            return
                columns
                    .Select((x, idx) => new { rawPropertyValue = propertySMecor(x), idx })
